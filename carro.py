...
cenario = GameImage("Imagens\Fase1completa.png")
...
level1 = GameImage ("Imagens\level1.png")
...
def game_loop(janela, mouse, teclado):
    ...
    inicio = True
    inicio_delay = 3 + janela.delta_time() *15
    inicio_tick = 0
    ...
    #Movimento Geral:
        ...
        #Movimento cenario:
        if cenario.y < 0:
            cenario.y += velcar *janela.delta_time() * 1000
            ...
            positiony -= velcar*janela.delta_time() * 1000
            if positiony <= 0:
                bloqueado = False
                fim = True
    ...
    #DRAW:
        ...
        if batido == True:
            ...
            explosao.draw()
            explosao.update()
        if (1.5 < inicio_tick < inicio_delay) and inicio == True:
            start.draw()
        if fim == True:
            gameover.draw()
        janela.update()